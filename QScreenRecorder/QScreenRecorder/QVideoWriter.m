...
- (id)initWithRecordable:(id<QRecordable>)recordable options:(QRecorderOptions)options {
    if ((self = [super init])) {
        ...
        if ((_options & QRecorderOptionUserCameraRecording) == QRecorderOptionUserCameraRecording) {
#if !TARGET_IPHONE_SIMULATOR
            _userRecorder = [[QCameraRecorder alloc] init];
        ...
        for (id<QRecordable> r in _recordables) {
            CGSize size = [r size];
...
- (BOOL)start:(NSError **)error {
    if (_writer) {
        QSetError(error, QErrorCodeInvalidState, @"Already started recording.");
        return NO;
    }
    
    _startTime = 0;
    _previousPresentationTime = kCMTimeNegativeInfinity;
    _video = [[QVideo alloc] init];
    ...
    if (!_writerInput) {
        QSetError(error, 0, @"Error with writer input");
        return NO;
    }
    ...
    if ((_options & QRecorderOptionUserAudioRecording) == QRecorderOptionUserAudioRecording) {
        // Add the audio input
    ...
    for (id<QRecordable> recordable in _recordables) {
...
- (BOOL)stop:(NSError **)error {
    if (!_writer) {
        QSetError(error, 0, @"Can't stop, its not running");
        return NO;
    }
    QDebug(@"Stopping");
    [self _stopTimer];
    ...
    if (_writer.status != AVAssetWriterStatusUnknown) {
        ...
        QDebug(@"Waiting to mark finished");
        NSUInteger i = 0;
        while (_writer.status == AVAssetWriterStatusUnknown) {
            [[NSRunLoop currentRunLoop] runUntilDate:[NSDate dateWithTimeIntervalSinceNow:0.1]];
            if (i++ > 100) {
                QWarn(@"Timed out waiting for writer to finish");
                break;
            }
        }
        ...
        if (!success) {
            QSetError(error, 0, @"Error finishing writing");
        }
    }
    
    if (_writer.error) {
        QWarn(@"Writer error: %@", _writer.error);
    }
...
- (BOOL)appendSampleBuffer:(CMSampleBufferRef)sampleBuffer {
    ...
    if (![_audioWriterInput isReadyForMoreMediaData]) {
        QDebug(@"Not ready for more data (audio)");
        return NO;
    }
    //CMTime time = CMSampleBufferGetPresentationTimeStamp(sampleBuffer);
    //QDebug(@"Audio timestamp: %0.2f", ((double)time.value * time.timescale));
...
- (BOOL)writeVideoFrameAtTime:(CMTime)time image:(CGImageRef)image error:(NSError **)error {
    ...
    if (![_writerInput isReadyForMoreMediaData]) {
        QDebug(@"Not ready for more data (video)");
        return NO;
    }
    ...
    if (!_pixelBuffer) {
        ...
        if (status != kCVReturnSuccess) {
            if (_writer.error) {
                ...
                QWarn(@"Error creating pixel buffer: %@", _writer.error);
            } else {
                QSetError(error, 0, @"Error creating pixel buffer");
            }
            return NO;
        }
    }
    ...
    if (_bufferAdapter && ![_bufferAdapter appendPixelBuffer:_pixelBuffer withPresentationTime:time]) {
        if (_writer.error) {
            if (error) *error = _writer.error;
            QWarn(@"Error appending pixel buffer: %@", _writer.error);
        } else {
            QSetError(error, 0, @"Error appending pixel buffer");
        }
        return NO;
    }
...
- (BOOL)_renderTouch:(QUITouch *)touch context:(CGContextRef)context {
...
- (void)_renderTouchesInContext:(CGContextRef)context {
    ...
    for (QUITouch *touch in _touches) {
    ...
    @synchronized(self) {
        ...
        for (UITouch *touch in [event allTouches]) {
            if (touch.phase == UITouchPhaseEnded) {
                ...
                QUITouch *crTouch = [[QUITouch alloc] initWithPoint:p];
...
- (BOOL)render:(NSError **)error {
    ...
    // Render touches
    if ((_options & QRecorderOptionTouchRecording) == QRecorderOptionTouchRecording) {
    ...
    if (_userRecorder) {
        ...
        if (CMTIME_IS_NEGATIVE_INFINITY(time)) {
            QDebug(@"No presentation time, skipping");
            return NO;
        }
    } else {
...
- (void)saveToAlbumWithName:(NSString *)name resultBlock:(QRecorderSaveResultBlock)resultBlock failureBlock:(QRecorderSaveFailureBlock)failureBlock {
    if (!_video) {
        if (failureBlock) failureBlock([QUtils cr_errorWithDomain:QErrorDomain code:QErrorCodeInvalidVideo localizedDescription:@"No video available to save."]);
        return;
    }
    
    QDebug(@"Saving to album");
