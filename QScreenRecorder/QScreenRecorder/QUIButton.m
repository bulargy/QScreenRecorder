...
- (void)sharedInit {
  ...
  _selectedShadingType = QUIShadingTypeUnknown;
  _highlightedShadingType = QUIShadingTypeUnknown;
  _disabledShadingType = QUIShadingTypeUnknown;
...
- (CGSize)_sizeForTitle:(NSString *)title constrainedToSize:(CGSize)constrainedToSize {
  ...
  if (_secondaryTitle) {
    if (_secondaryTitlePosition == QUIButtonSecondaryTitlePositionDefault || _secondaryTitlePosition == QUIButtonSecondaryTitlePositionRightAlign) {
    ...
    } else if (_secondaryTitlePosition == QUIButtonSecondaryTitlePositionBottom) {
      CGSize secondaryTitleSize = [_secondaryTitle sizeWithFont:(_secondaryTitleFont ? _secondaryTitleFont : _titleFont) constrainedToSize:constrainedToSize lineBreakMode:UILineBreakModeTailTruncation];
      titleSize.height += roundf(secondaryTitleSize.height);
    } else if (_secondaryTitlePosition == QUIButtonSecondaryTitlePositionBottomLeftSingle) {
...
- (void)layoutSubviews {
  ...
  if (!QCGSizeIsZero(_titleSize) && !_title) {
    ...
    if (_iconImageView.image && QCGSizeIsZero(iconSize)) {
    ...
    if (_activityIndicatorView) {
      if (_titleHidden) {
        CGPoint p = QCGPointToCenter(_activityIndicatorView.frame.size, size);
        ...
        CGPoint p = QCGPointToCenter(_titleSize, size);
...
- (UIEdgeInsets)_titleInsets {
  ...
  if (_borderStyle == QUIBorderStyleRoundedBack) {
    titleInsets.left += 4;
  }
  return titleInsets;
}
...
- (void)setColor:(UIColor *)color {
  ...
  if (_shadingType == QUIShadingTypeUnknown) {
    _shadingType = QUIShadingTypeNone;
  }
  [self didChangeValueForKey:@"color"];
}

- (void)_cornerRadiusChanged {
  if (_borderStyle == QUIBorderStyleNone && (_cornerRadius > 0 || _cornerRadiusRatio > 0)) {
    _borderStyle = QUIBorderStyleRounded;
...
- (void)sizeToFitWithMinimumSize:(CGSize)minSize {
  ...
  self.frame = QCGRectSetSize(self.frame, size);
}

- (void)setBorderStyle:(QUIBorderStyle)borderStyle {
...
- (void)drawInRect:(CGRect)rect {
  ...
  QUIShadingType shadingType = _shadingType;
  ...
  if (isDisabled) {
    if (_disabledShadingType != QUIShadingTypeUnknown) shadingType = _disabledShadingType;
    ...
    if (_highlightedShadingType != QUIShadingTypeUnknown) shadingType = _highlightedShadingType;
    ...
    if (_selectedShadingType != QUIShadingTypeUnknown) shadingType = _selectedShadingType;
    else if (_highlightedShadingType != QUIShadingTypeUnknown) shadingType = _highlightedShadingType;
  ...
  BOOL clip = (_borderStyle != QUIBorderStyleTopOnly && _borderStyle != QUIBorderStyleBottomOnly && _borderStyle != QUIBorderStyleTopBottom && _borderStyle != QUIBorderStyleNone && _borderStyle != QUIBorderStyleNormal);

  if (color && shadingType != QUIShadingTypeNone) {
    if (clip) {
      CGContextSaveGState(context);
    }

    QCGContextAddStyledRect(context, bounds, _borderStyle, borderWidth, cornerRadius);
    if (clip) {
      CGContextClip(context);
    }

    QCGContextDrawShading(context, color.CGColor, color2.CGColor, color3.CGColor, color4.CGColor, bounds.origin, CGPointMake(bounds.origin.x, CGRectGetMaxY(bounds)), shadingType, NO, NO);
    fillColor = nil;
  ...
  if (_borderWidth > 0 || cornerRadius > 0) {
    if (borderShadowColor) {
      ...
      if (clip) {
        QCGContextAddStyledRect(context, bounds, _borderStyle, 0, cornerRadius);
        CGContextClip(context);
      }

      QCGContextDrawBorderWithShadow(context, bounds, _borderStyle, fillColor.CGColor, borderColor.CGColor, borderWidth, cornerRadius, borderShadowColor.CGColor, borderShadowBlur, NO);
      CGContextRestoreGState(context);
    } else {
      QCGContextDrawBorder(context, bounds, _borderStyle, fillColor.CGColor, borderColor.CGColor, borderWidth, cornerRadius);
    }
  } else if (fillColor) {
    [fillColor setFill];
  ...
  CGFloat y = bounds.origin.y + roundf(QCGPointToCenter(titleSize, size).y) + _insets.top;

  BOOL showIcon = (icon != nil && !_iconImageView.hidden);
  CGSize iconSize = _iconImageSize;
  if (icon && QCGSizeIsZero(iconSize)) {
    iconSize = icon.size;
  }
  ...
  if (!_titleHidden) {
    ...
    if (showIcon && _iconPosition == QUIButtonIconPositionLeft) lineWidth += iconSize.width;
    ...
    if (showIcon) {
      ...
      switch (_iconPosition) {
        case QUIButtonIconPositionLeft: {
          CGPoint iconTop = QCGPointToCenter(iconSize, size);
          iconTop.x = x;
        ...
        case QUIButtonIconPositionCenter: {
          CGPoint iconTop = QCGPointToCenter(iconSize, CGSizeMake(size.width, size.height - titleSize.height));
      ...
      showIcon = NO;
    } else if (!QCGSizeIsZero(iconSize)) {
      if (_iconPosition == QUIButtonIconPositionLeft) {
        x += iconSize.width;
      }
    }
    ...
    if (!_secondaryTitle) {
      ...
      if (_secondaryTitlePosition == QUIButtonSecondaryTitlePositionDefault) {
        x += titleSizeAdjusted.width;
        [_secondaryTitle drawAtPoint:CGPointMake(x, y) withFont:font];  
      } else if (_secondaryTitlePosition == QUIButtonSecondaryTitlePositionRightAlign) {
        x += titleSizeAdjusted.width;
  ...
  if (accessoryImage) {
    [accessoryImage drawAtPoint:QCGPointToRight(accessoryImage.size, CGSizeMake(size.width - 10, bounds.size.height))];
  }

  if (showIcon) {
    if (_iconShadowColor) CGContextSetShadowWithColor(context, CGSizeZero, 3.0, _iconShadowColor.CGColor);
    [icon drawInRect:QCGRectToCenterInRect(iconSize, bounds)];
    CGContextSetShadowWithColor(context, CGSizeZero, 0.0, NULL);
  }
}
