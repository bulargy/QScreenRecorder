...
typedef enum {
  QUIButtonStyleDefault = 1,
  QUIButtonStylePrimary,
  QUIButtonStyleStart,
  QUIButtonStyleWarning,
  QUIButtonStyleDanger,
} QUIButtonStyle;

@implementation QUIRecordOverlay
...
- (id)initWithFrame:(CGRect)frame {
  if ((self = [super initWithFrame:frame])) {
    ...
    _recordBackground = [[QUIButton alloc] init];
    ...
    _startButton = [self buttonWithTitle:@"Start Recording" action:@selector(_start) style:QUIButtonStyleStart];
    [_recordBackground addSubview:_startButton];
    
    /*
    _resumeButton = [self buttonWithTitle:@"Resume" action:@selector(_resume) style:QUIButtonStyleStart];
    _resumeButton.hidden = YES;
    [_recordBackground addSubview:_resumeButton];
    
    _pauseButton = [self buttonWithTitle:@"Pause" action:@selector(_pause) style:QUIButtonStylePause];
    _pauseButton.hidden = YES;
    [_recordBackground addSubview:_pauseButton];
     */
    
    _stopButton = [self buttonWithTitle:@"Stop Recording" action:@selector(_stop) style:QUIButtonStyleDanger];
    _stopButton.hidden = YES;
    [_recordBackground addSubview:_stopButton];
    
    _closeButton = [self buttonWithTitle:@"Close" action:@selector(_close) style:QUIButtonStyleDefault];
    ...
    _saveBackground = [[QUIButton alloc] init];
    ...
    _saveButton = [self buttonWithTitle:@"Save Video" action:@selector(_save) style:QUIButtonStylePrimary];
    [_saveBackground addSubview:_saveButton];
    
    _discardButton = [self buttonWithTitle:@"Discard Video" action:@selector(_discard) style:QUIButtonStyleWarning];
    [_saveBackground addSubview:_discardButton];
    
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(_updateForVideoNotification:) name:QVideoDidChangeNotification object:nil];
  }
  return self;
}
...
- (QUIButton *)buttonWithTitle:(NSString *)title action:(SEL)action style:(QUIButtonStyle)style {
  QUIButton *button = [[QUIButton alloc] initWithFrame:CGRectZero title:title target:self action:action];
  button.borderWidth = 1.0;
  button.cornerRadius = 6.0;
  button.titleFont = [UIFont boldSystemFontOfSize:16.0];
  button.shadingType = QUIShadingTypeLinear;
  button.highlightedShadingType = QUIShadingTypeLinear;
  button.disabledTitleShadowColor = [UIColor colorWithWhite:0 alpha:0];
  button.disabledShadingType = QUIShadingTypeNone;
...
- (void)_start {
  if (![[QRecorder sharedRecorder] isRecording]) {
    [[QRecorder sharedRecorder] start:nil];
    [self _close:NO];
  }
}
...
- (void)_stop {
  if ([[QRecorder sharedRecorder] isRecording]) {
    [[QRecorder sharedRecorder] stop:nil];
  } else {
    [self _close:NO];
  }
}
...
- (void)_save {
  [[QRecorder sharedRecorder] saveVideoToAlbumWithResultBlock:^(NSURL *URL) {
...
- (void)_discardConfirmed {
  [[QRecorder sharedRecorder] discardVideo:nil];
  [self _close:NO];
}
...
- (void)_updateForVideoNotification:(NSNotification *)notification {
  QVideo *video = [notification object];
...
- (void)_updateForStatus:(QVideoStatus)status {
  switch (status) {
    case QVideoStatusNone:
      [self _reset];
      break;
    case QVideoStatusStarted:
      _recordBackground.hidden = NO;
      _saveBackground.hidden = YES;
      _startButton.hidden = YES;
      _stopButton.hidden = NO;
      break;
    case QVideoStatusStopped:
      _recordBackground.hidden = YES;
      _saveBackground.hidden = NO;
      _saveButton.title = @"Save Video";
      _saveButton.enabled = YES;
      break;
    case QVideoStatusSaving:
      _saveButton.title = @"Saving...";
      _saveButton.enabled = NO;
      break;
    case QVideoStatusSaved:
      [self _reset];
      [self _close:NO];
      break;
    case QVideoStatusDiscarded:
      [self _reset];
      [self _close:NO];
      break;
  }
