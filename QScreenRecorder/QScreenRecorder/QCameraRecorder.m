...
@implementation QCameraRecorder
...
- (BOOL)start:(NSError **)error {
  if (_captureSession) {
    QSetError(error, 0, @"Capture session already started");
    return NO;
  }
  ...
  if (audioCaptureDevice) {
    ...
    QDebug(@"Adding audio input: %@", audioInput);
  ...
  if (![_captureSession canAddOutput:_videoOutput]) {
    QSetError(error, 0, @"Can't add video output: %@", _videoOutput);
    return NO;
  }
  ...
  QDebug(@"Starting capture session...");
  [_captureSession startRunning];
  QDebug(@"Started capture session");
  return YES;
}
...
- (BOOL)close {
  ...
  QDebug(@"Closing capture session");
  ...
  while (_captureSession.isRunning) {
    ...
    if (i++ > 100) {
      QWarn(@"Timed out waiting for capture session to close");
      break;
    }
  }
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
  ...
  if (_data == NULL || dataSize != _dataSize) {
    QDebug(@"Allocating video data of size: %ld", dataSize);
